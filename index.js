...
bot.onText(/\/start/, msg => {
  ...
  const name = msg.from.first_name || "Chaos Gremlin";
...
💎🚨 **GRAND LUCKY SPIN ALERT** 🚨💎
💸 Entry Fee: **$20 shards only**
🎰 Spin the wheel for a chance to WIN:
📱 **iPhone 17 Pro Max** 📱
🏆 Limited slots — first come, first served!
⏰ Act fast! Don’t miss this ultimate prize!
...
💞 **Other Fun Chaos:**
🎰 Spin the Wheel of Shame & WIN extra shards!
💘 Find your soulmate & flirt your way to glory!
🧠 VIP Quiz: Test your chaos IQ & earn Rizz points!
💌 Chat with stunning NPCs & send love tricks!

⚡ Daily Chaos Bonus: ${bonus ? "💎 +$5 shards collected!" : "💎 Claim every day!"}

🎯 Tap buttons below to start your chaotic adventure & grab your chance for the **iPhone 17 Pro Max**! 👇
`;

  // Promo image (replace with your hosted iPhone promo image)
...
if(data==="grandChance"){
  ...
  // Entry confirmation
  await bot.sendMessage(uid, "🍀 **Lucand Grand Chance entry confirmed!** Spinning the wheel… 🎰", { parse_mode:"Markdown" });

  await new Promise(r => setTimeout(r, 2000));

  const win = Math.random() < 0.05; // 5% chance to win iPhone
  ...
  if(win){
    ...
    const winMessage = `
🎉🎉🎉 **JACKPOT!** 🎉🎉🎉
📱 You just WON the **iPhone 17 Pro Max!** 📱
💸 Entry Fee: $20 shards
🏆 Celebrate your ultimate luck, ${p.name}! 🎊
🔥 VIP users have extra spins to increase chances!
🎰 Spin again for more rewards or claim your prize!
    `;
    
    // Optional celebration GIF
    const celebrationGIF = "https://media.giphy.com/media/111ebonMs90YLu/giphy.gif"; // confetti GIF

    await bot.sendAnimation(uid, celebrationGIF, { caption: winMessage, parse_mode:"Markdown", reply_markup: mainMenu(p) });
    
  } else {
    // Near miss message
    await bot.sendMessage(uid, "💀 Almost… luck wasn’t on your side this time. Try again next week! 🍀", { reply_markup: mainMenu(p) });
  }
  }
