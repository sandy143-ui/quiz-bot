...
function getPlayer(uid, name="Chaos Gremlin") {
  if (!players[uid]) {
    players[uid] = {
      ...
      freeChats: 0   // ✅ allow 2-3 free NPC chats
    };
    saveDB();
  }
  return players[uid];
...
bot.on("callback_query", async q=>{
  try {
    ...
    if(data==="menu"){
      await bot.sendMessage(uid,"Back to main menu",{reply_markup:mainMenu(p)});
      return;
    }
...
Roast mode: ${p.roastMode?"ON 🔥":"OFF (coward)"}`
    ...
    if(data==="quiz" && quizQuestions.length){
      ...
      await bot.sendMessage(uid, `🧠 *CHAOS QUIZ DROP*\n\n${qn.q}`, { reply_markup: kb, parse_mode:"Markdown" });
    }
    ...
    if(data.startsWith("qans_")){
      ...
      await bot.sendMessage(uid,"Menu?", { reply_markup: mainMenu(p) });
      saveDB();
    }
    ...
    if(data==="match"){
      ...
      await chaosExplosion(uid, `✨ MATCH FOUND ✨\nCompatibility: ${perc}%\nPartner → ${partner.name}\n${line}${roast}`);
    ...
    // ─ WHEEL ─
    if(data==="wheel"){
      if(!hasVIP(uid)) return bot.sendMessage(uid, "💔 VIP only! Upgrade to spin the Wheel of Shame.");
      ...
      await chaosExplosion(uid, amt>0?`You won +$${amt} shards! 🔥`:amt<0?`You LOST $${-amt} shards 💀`:"Zero. Try again 🤡");
    }
    ...
    if(data==="deposit"||data==="vip"){
      ...
      await bot.sendMessage(uid,data==="deposit"?"Choose your rizz boost:":"Choose VIP status:",{reply_markup:kb});
    }
    ...
    if(data.startsWith("chat_")){
      ...
      if(npc){
        // Allow 3 free chats for non-VIP
        if(!hasVIP(uid) && p.freeChats >= 3){
    ...
    if(data==="locked"){
      await bot.sendMessage(uid,"VIP only 😤 Deposit or perish.");
    }
...
bot.onText(/\/add (\d+) (\d+)/, (msg, match) => {
  try {
    ...
    if(players[uid]){
      players[uid].wallet += amt;
      if(amt >= 10) players[uid].vip = true;
      saveDB();
