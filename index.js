...
function generateFemaleUsers(count = 30) {
  const names = ["Emma","Olivia","Ava","Sophia","Isabella","Mia","Charlotte","Amelia","Harper","Evelyn"];
  ...
  for (let i=0;i<count;i++){
    ...
    femaleUsers.push({uid: generateUID(), name, loves: [], lastChat: Date.now()});
  }
}
generateFemaleUsers();
...
Roast mode: ${p.roastMode?"ON 🔥":"OFF (coward)"}`
  ...
  if(data==="quiz" && quizQuestions.length){
    ...
    const opts=[...qn.wrong, qn.correct].sort(()=>Math.random()-0.5);
    const kb={inline_keyboard: opts.map((t,i)=>[{text:t,callback_data:`qans_${i}_${t===qn.correct?"W":"L"}` }])};
    bot.sendMessage(uid, `🧠 CHAOS QUIZ DROP\n\n${qn.question}`, {reply_markup: kb});
  }

  if(data.startsWith("qans_")){
    const [, , res]=data.split("_");
    p.quizPlayed++;
    if(res==="W"){p.quizScore+=15;p.flirtPoints+=8;p.wins++; await chaosExplosion(uid,"CORRECT! You got Rizz! +15 pts +8 flirt",1800);}
  ...
  if(data==="match"){
    ...
    if(matchmakingQueue.includes(uid)) return bot.sendMessage(uid,"Already simping... wait 💅");
    ...
    if(matchmakingQueue.length>=2 && matchmakingQueue[0]===uid){
      const partner=matchmakingQueue.find(i=>i!==uid);
      if(partner){
        matchmakingQueue=matchmakingQueue.filter(i=>i!==uid && i!==partner);
        ...
        chaosExplosion(partner,`✨ MATCH FOUND ✨\nCompatibility: ${perc}%\nVictim → @user${uid}\n${randomPickup()}`);
        p.couples++; players[partner].couples++; p.flirtPoints+=25; players[partner].flirtPoints+=25;
        updateRank(p); updateRank(players[partner]); saveDB();
      }
    } else setTimeout(()=>{
      if(matchmakingQueue.includes(uid)) {bot.sendMessage(uid,"No one wants you yet 💔"); matchmakingQueue=matchmakingQueue.filter(i=>i!==uid);}
    },15000);
  }
  ...
  if(data.startsWith("chat_")){
    ...
    if(npc){
      ...
      setTimeout(()=>bot.sendMessage(uid,`✨ ${npc.name} seems interested! 💖 Upgrade to VIP for more matches!`),7000);
    }
  }

  if(data==="locked"){bot.sendMessage(uid,"VIP only 😤 Deposit or perish.");}
});

// ─── ADMIN ADD SHARDS ─────────────────────────────────────
bot.onText(/\/add (\d+) (\d+)/,(msg,[uid,amt])=>{
  if(msg.from.id!==ADMIN_ID) return;
  if(players[uid]){players[uid].wallet+=Number(amt); if(Number(amt)>=10)players[uid].vip=true; saveDB(); bot.sendMessage(uid,`Admin gift +$${amt} 💸`);}
});
