...
let participants = [];
let vipParticipants = [];
let pendingPayments = {}; // keys are STRING
let lastAction = {};
let couples = [];
...
bot.on("callback_query", async (q) => {
  const id = String(q.message.chat.id); // store as string for approve
  if (!antiSpam(id)) return bot.answerCallbackQuery(q.id, { text: "⏳ Slow down, superstar!" });

  // STANDARD JOIN
  if (q.data === "join_standard") {
    pendingPayments[id] = { type: "standard" }; // string key
...
🎉💖🎮✨🎆🎊💫🎈🌟🔥`, { parse_mode: "Markdown" });
  ...
  // VIP JOIN
  if (q.data === "join_vip") {
    pendingPayments[id] = { type: "vip" }; // string key
    return bot.sendMessage(id,
  ...
  if (q.data.startsWith("answer_")) {
    ...
    if (correct) {
      ...
      blastMessages.forEach((msgText, i) => {
        setTimeout(() => bot.sendMessage(playerId, msgText, { parse_mode: "Markdown" }), i * 1000);
      });
    }
...
bot.onText(/\/approve (\d+)/, (msg, match) => {
  ...
  const userId = String(match[1]); // always string
...
🎉💖🎮✨🎆🎊💫🎈🌟🔥`);
  ...
  saveProfiles();
  bot.sendMessage(msg.chat.id, `✅ Approved successfully → ${userId}`);
});
