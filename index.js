...
function menu(p) {
  return {
    inline_keyboard: [
      ...
      [{ text: "💎 Join VIP", callback_data: "pay_vip" }],
      [{ text: "✨ Join Standard", callback_data: "pay_standard" }]
    ]
  };
}
...
bot.on("callback_query", async (q) => {
  ...
  // ================= VIP PAYMENT ==================
  if (data === "pay_vip") {
    const link = `https://api.zina.io/pay?amount=${VIP_FEE}&user=${id}&methods=card,applepay,googlepay,usdt`;
    pendingPayments[id] = { type: "vip", amount: VIP_FEE, link, approved: false };
    bot.sendMessage(id, `💎 VIP Payment\nPay $${VIP_FEE} via Zina:\n${link}\n\nAfter payment, wait for admin approval.`);
  }

  // ================= STANDARD PAYMENT ==================
  if (data === "pay_standard") {
    const link = `https://api.zina.io/pay?amount=${STANDARD_FEE}&user=${id}&methods=card,applepay,googlepay,usdt`;
    pendingPayments[id] = { type: "standard", amount: STANDARD_FEE, link, approved: false };
    bot.sendMessage(id, `✨ Standard Payment\nPay $${STANDARD_FEE} via Zina:\n${link}\n\nAfter payment, wait for admin approval.`);
  }
  ...
  if (data === "spin") {
    ...
    bot.sendMessage(id, `🎡 Spinning Galaxy Wheel…\n\nYou Won: 💰 $${prize}`);
  }
  ...
  if (data === "battle") {
    ...
    bot.sendMessage(id, win ? "⚡ Neon Victory +$3 🏆" : "💀 Cyber Defeat");
  }
  ...
  // ================= VIP ==================
  if (data === "vip") {
    bot.sendMessage(id, `👑 VIP UPGRADE\n\nUnlock All Realms\nBecome Royalty Forever`);
  }
...
// ================= LOCKED ====================
function locked(id) {
  bot.sendMessage(id, "🔒 This Realm Is Locked\nUpgrade To Enter 💎");
}
...
bot.onText(/\/ai_love/, (msg) => {
  const id = msg.chat.id;
  bot.sendMessage(id, "💘 AI Romance Activated…\n\n🤖 Your virtual partner whispers:\n‘You look stunning tonight… ready to win hearts together?’ 💞");
});

bot.onText(/\/ai_dare/, (msg) => {
  const id = msg.chat.id;
  const dares = ["Send a sweet voice note 🎙","Confess your crush 😏","Drop a heart bomb in chat 💣❤️","Say something romantic in 5 seconds ⏳"];
...
// Approve payment by userId
bot.onText(/\/approve (\d+)/, (msg, match) => {
  if (msg.from.id !== ADMIN_ID) return bot.sendMessage(msg.chat.id, "⛔ Admin Only");
  const userId = match[1];
  if (!pendingPayments[userId]) return bot.sendMessage(msg.chat.id, "No pending payment for this user.");
  ...
  else if (payment.type === "standard") p.standard = true;
  payment.approved = true;
  delete pendingPayments[userId];
  saveProfiles();
  bot.sendMessage(userId, `✅ Payment approved! You are now ${payment.type.toUpperCase()} member!`);
  bot.sendMessage(msg.chat.id, `Approved ${payment.type} for ${p.name} (${userId})`);
});

// Approve payment by username
bot.onText(/\/approve_name (.+)/, (msg, match) => {
  ...
  else if (payment.type === "standard") userEntry.standard = true;
  payment.approved = true;
  delete pendingPayments[userId];
  saveProfiles();
  bot.sendMessage(userId, `✅ Payment approved! You are now ${payment.type.toUpperCase()} member!`);
  bot.sendMessage(msg.chat.id, `Approved ${payment.type} for ${userEntry.name} (${userId})`);
});
