...
async function sendVIPWelcome(userId) {
  ...
  await bot.sendAnimation(userId, gif, { caption: "💎 Welcome VIP! Enjoy luxury, thrill & love 🌹✨" });
}
...
// ================= START COMMAND (Promotion-first flashy) =================
bot.onText(/\/start/, async (msg) => {
  ...
  const gif = gifs[Math.floor(Math.random() * gifs.length)];
  await bot.sendAnimation(id, gif, { caption: "✨ Welcome to *QUIZ ARENA ELITE* ✨\nYour adventure begins now! 💖🎮🔥" , parse_mode: "Markdown"});
...
Pay here: [Click to Pay](${link})`, { parse_mode: "Markdown", disable_web_page_preview: true });
  ...
  if (q.data === "couples_request") {
    ...
    if (waiting) {
      ...
      bot.sendMessage(Number(id), `💖 Paired with ${profiles[otherId].name}! Start secret chat 💌`);
      bot.sendMessage(Number(otherId), `💖 Paired with ${profiles[id].name}! Start secret chat 💌`);

      const videoLink = `https://yourserver.com/video_call/${id}_${otherId}`;
      bot.sendMessage(Number(id), `🎥 Secret video call: [Click to Join](${videoLink})`);
      bot.sendMessage(Number(otherId), `🎥 Secret video call: [Click to Join](${videoLink})`);
    } else {
      couples[id] = null;
